...
def check_all_symbols():
    print(f"\n[{datetime.now().strftime('%H:%M:%S')}] Verification des news...")
    alerts = []
    for symbol in WATCHLIST:
        ...
        for news in news_list:
            ...
            news['datetime'] = datetime.fromtimestamp(news.get('datetime', 0)).strftime('%d/%m/%Y %H:%M') if news.get('datetime') else 'N/A'
            if price:
                news['price'] = price
            if analysis.get('confiance', 0) >= ALERT_THRESHOLD:
                message = format_alert(symbol, news, analysis)
                send_telegram(message)
            alerts.append({"symbol": symbol, "price": price, "headline": headline, "analysis": analysis, "datetime": news['datetime']})
    return alerts
