...
def save_mentor_message(role, content):
    conn = get_conn()
    conn.execute(
        "INSERT INTO mentor_memory (role, content) VALUES (?, ?)",
        (role, content)
    )
    conn.commit()
    conn.close()
...
def get_mentor_history(limit=20):
    conn = get_conn()
    rows = conn.execute(
        "SELECT role, content FROM mentor_memory ORDER BY created_at DESC LIMIT ?",
        (limit,)
    ).fetchall()
    conn.close()
...
def save_custom_alert(symbol, condition, target_price):
    conn = get_conn()
    conn.execute("""
        INSERT INTO custom_alerts (symbol, condition, target_price)
        VALUES (?, ?, ?)
    """, (symbol.upper(), condition, target_price))
    conn.commit()
    conn.close()
...
def get_custom_alerts():
    conn = get_conn()
    rows = conn.execute(
        "SELECT * FROM custom_alerts ORDER BY created_at DESC"
    ).fetchall()
    conn.close()
    return [dict(r) for r in rows]
...
def mark_alert_triggered(alert_id):
    conn = get_conn()
    conn.execute("UPDATE custom_alerts SET triggered=1 WHERE id=?", (alert_id,))
    conn.commit()
    conn.close()
...
def get_untriggered_alerts():
    conn = get_conn()
    rows = conn.execute(
        "SELECT * FROM custom_alerts WHERE triggered=0"
    ).fetchall()
    conn.close()
    return [dict(r) for r in rows]
...
def save_investment_plan(budget, risk, goal, horizon, plan_json):
    import json
    conn = get_conn()
    conn.execute("""
        INSERT INTO investment_plans (budget, risk, goal, horizon, plan_json)
        VALUES (?, ?, ?, ?, ?)
    """, (budget, risk, goal, horizon, json.dumps(plan_json)))
    conn.commit()
    conn.close()

def get_latest_investment_plan():
    import json
    conn = get_conn()
    row = conn.execute(
        "SELECT * FROM investment_plans ORDER BY created_at DESC LIMIT 1"
    ).fetchone()
    conn.close()
    if row:
        d = dict(row)
